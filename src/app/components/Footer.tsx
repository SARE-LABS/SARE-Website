  ...
  */}
  <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 lg:gap-8 mb-16">
    ...
    <div className="lg:col-span-1 flex flex-col items-center lg:items-start text-center lg:text-left w-full">
      <Image src={logoTwo} alt="SARE Logo" width={180} height={80} className="mb-6" />
      ...
      {footerLinks.map((column, idx) => (
        <div key={idx} className="flex flex-col gap-4 md:gap-6">
          <h4 className="flex items-center gap-2 font-bold text-[#1E293B] text-[16px] md:text-[18px]">
            <span className="text-[12px] font-normal">→</span> {column.title}
          </h4>
  ...
  <div className="border-t border-gray-200 pt-8 flex flex-col md:flex-row justify-between items-center gap-6">
    ...
    <div className="flex items-center gap-3 flex-wrap justify-center">
      ...
      <Link href="https://www.linkedin.com/company/society-of-agricultural-robotics-engineers/" className="w-10 h-10 rounded-full bg-[#F1F5F9] flex items-center justify-center text-gray-500 hover:bg-[#67B5DC] hover:text-white transition-all">
        <Image src={Linkdln} alt="LinkedIn" width={16} height={16} className="opacity-60" />
      </Link>
    </div>
  </div>
</div>
